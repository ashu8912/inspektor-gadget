...
func Notify(runtimePath string, hostRoot string, notifyFD *fanotify.NotifyFD) (string, error) {
	...
	if !strings.HasPrefix(runtimePath, hostRoot) {
		...
		runtimePath, err := securejoin.SecureJoin(hostRoot, runtimePath)
		if err != nil {
			return "", fmt.Errorf("securejoining of %s: %w", runtimePath, err)
		}
	}
	...
	if err := notifyFD.Mark(unix.FAN_MARK_ADD, unix.FAN_OPEN_EXEC_PERM, unix.AT_FDCWD, runtimePath); err != nil {
		return "", fmt.Errorf("marking of %s: %w", runtimePath, err)
	}
