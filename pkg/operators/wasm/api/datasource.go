...
func (ds DataSource) Release(packet Packet) {
	dataSourceRelease(uint32(ds), uint32(packet))
}
...
func (d DataArray) Append(data Data) {
	dataArrayAppend(uint32(d), uint32(data))
}

func (d DataArray) Release(data Data) {
	dataArrayRelease(uint32(d), uint32(data))
}
