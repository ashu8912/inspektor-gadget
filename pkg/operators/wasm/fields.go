...
func (i *wasmOperatorInstance) fieldAccessorSet(ctx context.Context, m wapi.Module, stack []uint64) {
	...
	switch api.Kind(stack[2]) {
	case api.Kind_Bool:
		err = acc.PutBool(data, stack[3] != 0)
	case api.Kind_Int8:
		err = acc.PutInt8(data, int8(stack[3]))
	case api.Kind_Int16:
		err = acc.PutInt16(data, int16(stack[3]))
	case api.Kind_Int32:
		err = acc.PutInt32(data, int32(stack[3]))
	case api.Kind_Int64:
		err = acc.PutInt64(data, int64(stack[3]))
	case api.Kind_Uint8:
		err = acc.PutUint8(data, uint8(stack[3]))
	case api.Kind_Uint16:
		err = acc.PutUint16(data, uint16(stack[3]))
	case api.Kind_Uint32:
		err = acc.PutUint32(data, uint32(stack[3]))
	case api.Kind_Uint64:
		err = acc.PutUint64(data, uint64(stack[3]))
	case api.Kind_Float32:
