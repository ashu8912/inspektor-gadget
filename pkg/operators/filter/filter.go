...
func (f *filterOperatorInstance) PreStart(gadgetCtx operators.GadgetContext) error {
	for ds, funcs := range f.ffns {
		...
		ds.Subscribe(func(ds datasource.DataSource, data datasource.Data) error {
			...
			return nil
		}, 49000) // TODO: need some predefined & sane values
	}
	return nil
}
	...
	for {
		...
		case 1:
			...
			default:
				...
				default:
					return fmt.Errorf("invalid operation: %q", opString)
				}
				stage = 2
			}
		case 2:
			value += string(filter[pos])
			pos++
		}
	}
...
func getFilterFunc(f datasource.FieldAccessor, op comparisonType, negate bool, stringVal string) (func(datasource.DataSource, datasource.Data) bool, error) {
	var fn func(datasource.DataSource, datasource.Data) bool
	var intVal int64
	var uintVal uint64
	...
	if fieldType == api.Kind_String && op == comparisonTypeRegex {
		return nil, fmt.Errorf("regex based filtering can only be used for string types")
	}
	...
	case api.Kind_String, api.Kind_Invalid:
	...
	case api.Kind_String, api.Kind_CString:
		...
		fn = func(ds datasource.DataSource, data datasource.Data) bool {
			v, _ := f.String(data)
